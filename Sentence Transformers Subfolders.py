...
class RobustEmbeddingGenerator:
    ...
    def find_all_jsonl_files_recursive(self, folder_path: str) -> List[Path]:
        """Find all .jsonl files recursively in ALL subfolders"""
        folder_path = Path(folder_path)

        if not folder_path.exists():
            raise FileNotFoundError(f"Folder not found: {folder_path}")

        # Use rglob for recursive search through all subfolders
        jsonl_files = list(folder_path.rglob("*.jsonl"))  # ✅ Recursive search

        if not jsonl_files:
            raise FileNotFoundError(f"No .jsonl files found in: {folder_path} or its subfolders")

        print(f"📁 Found {len(jsonl_files)} .jsonl files across all subfolders:")
        ...
        for file in jsonl_files:
            ...
            files_by_folder[parent_folder].append(file.name)
    ...
    def load_jsonl_file_robust(self, jsonl_path: Path) -> List[Dict[str, Any]]:
        ...
        relative_path = jsonl_path.relative_to(Path.cwd()) if jsonl_path.is_absolute() else jsonl_path
    ...
    def load_all_jsonl_files(self, folder_path: str) -> Dict[str, List[Dict[str, Any]]]:
        """Load all .jsonl files recursively from the folder and all subfolders"""
        jsonl_files = self.find_all_jsonl_files_recursive(folder_path)
        all_data = {}
        ...
        for jsonl_file in jsonl_files:
            ...
            if documents:
                ...
                relative_key = str(jsonl_file.relative_to(Path(folder_path)))
                all_data[relative_key] = documents

        total_docs = sum(len(docs) for docs in all_data.values())
        print(f"🎉 Total documents loaded: {total_docs} from {len(all_data)} files across all subfolders")
        return all_data
    ...
    def generate_embeddings_for_folder(self, folder_path: str, output_dir: str = None) -> None:
        """Generate embeddings from ALL .jsonl files in folder and ALL subfolders"""
        # Load all documents from all .jsonl files recursively
        ...
        for file_path, documents in all_file_data.items():
            print(f"📝 Processing {len(documents)} documents from {file_path}")

            for local_index, doc in enumerate(documents):
                # Validate document structure
                if not isinstance(doc, dict):
                    print(f"⚠️ Skipping non-dict object in {file_path} at index {local_index}")
                    continue

                embedding_text = self.extract_text_for_embedding(doc)
                if not embedding_text:
                    print(f"⚠️ Empty text in {file_path}, document {local_index}, skipping")
                    continue
                ...
                metadata.append({
                    ...
                    'source_file': file_path,  # Now includes subfolder path
    ...
    def save_embeddings(self, folder_path: str, embeddings: np.ndarray,
        ...
        if output_dir is None:
            output_dir = folder_path / "all_embeddings_recursive"
        else:
        ...
        # Save model info with subfolder statistics
        unique_folders = set()
        for meta in metadata:
            source_path = Path(meta['source_file'])
            if len(source_path.parts) > 1:  # Has subfolder
                unique_folders.add(source_path.parent)
        ...
        info_data = {
            ...
            'num_subfolders_processed': len(unique_folders),
            'subfolders_list': sorted([str(folder) for folder in unique_folders]),
        ...
        print(f"📂 Subfolders processed: {info_data['num_subfolders_processed']}")
...
def main():
    """Generate embeddings from ALL .jsonl files recursively in folder tree"""

    print("🧠 Recursive Embedding Generator - Step 2 of 4")
    print("=" * 60)
    print("Creating embeddings from ALL .jsonl files in root folder AND subfolders")
    print("Uses brace-counting to extract ALL JSON objects without skipping")
    print("Searches recursively through entire folder tree")
    print("=" * 60)
    ...
    try:
        ...
        # Get folder path containing .jsonl files (will search recursively)
        folder_path = input("\n📁 Enter path to ROOT folder (will search all subfolders): ").strip().strip('"\'')
        ...
        # Generate embeddings for ALL .jsonl files in the folder tree
        generator.generate_embeddings_for_folder(folder_path)

        print(f"\n✅ SUCCESS! Embeddings created from all .jsonl files in entire folder tree.")
        print(f"📋 Next steps:")
